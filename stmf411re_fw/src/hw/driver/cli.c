...
bool cliRunCmd(cli_t *p_cli)
{
  ...
  if (cliParseArgs(p_cli) == true) /*string input confirm  엔터 치는 순간 파싱*/
  {
    cliPrintf("\r\n");

    cliToUpper(p_cli->argv[0]);/*대문자로 변환*/

    for (int i=0; i<p_cli->cmd_count; i++)
    {
      if (strcmp(p_cli->argv[0], p_cli->cmd_list[i].cmd_str) == 0) /*명령어 문자열과 방금들어온 string 과 비교 */
      {
        p_cli->cmd_args.argc =  p_cli->argc - 1;
        p_cli->cmd_args.argv = &p_cli->argv[1]; /*콜백함수에서 명령어 뒤에 오는 payload 부터*/
        p_cli->cmd_list[i].cmd_func(&p_cli->cmd_args); /*callback 함수 호출*/
        break;
      }// YOUTUBE 15:49 앞으로는 코드에 주석하면서 이해하고 총정리를 memo_log 에 정리할 것
    }
  }
